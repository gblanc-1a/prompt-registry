...
suite('BundleCommands - Check Bundle Updates Fix', () => {
    ...
    suite('checkAllUpdates() - Existing Behavior', () => {
        test('should check for updates and show selection dialog when updates available', async () => {
            ...
            assert.strictEqual(
                mockRegistryManager.checkUpdates.callCount,
                1,
                'Should call checkUpdates'
            );
        ...
        test('should show "up to date" message when no updates available', async () => {
            ...
            assert.strictEqual(
                mockShowInformationMessage.getCall(0).args[0],
                'All bundles are up to date!'
            );
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                0,
                'Should not call updateBundle'
            );
        });
    });
    ...
    suite('Bug Reproduction - Original Issue', () => {
        test('REPRODUCES BUG: updateBundle() directly installs without showing options', async () => {
            ...
            assert.strictEqual(
                updateBundleSpy.callCount,
                1,
                'BUG: updateBundle was called directly'
            );
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                1,
                'BUG: Bundle was updated without user confirmation'
            );
            ...
            assert.strictEqual(
                ...
                0,
                'BUG: No selection dialog was shown'
            );
        });
        ...
        test('DEMONSTRATES: old vs new command behavior', async () => {
            ...
            assert.strictEqual(
                mockShowInformationMessage.callCount,
                1,
    ...
    suite('Fixed Behavior - checkSingleBundleUpdate()', () => {
        test('FIXED: shows update dialog with options before updating', async () => {
            ...
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                1,
                'Should call updateBundle after user confirmation'
            );
        });
        ...
        test('FIXED: does not update when user cancels', async () => {
            ...
            const bundleId = 'test-bundle';
            setupUpdateAvailable(mockRegistryManager, bundleId);
            mockShowInformationMessage.resolves(); // Modal dialog returns undefined when cancelled
            ...
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                0,
                'Should not call updateBundle when cancelled'
            );
        });
        ...
        test('FIXED: handles "View Details" option', async () => {
            ...
            assert.strictEqual(
                executeCommandSpy.getCall(0).args[0],
                'promptRegistry.viewBundle',
                'Should execute correct command'
            );
            assert.strictEqual(
                executeCommandSpy.getCall(0).args[1],
                bundleId,
                'Should pass correct bundle ID'
            );
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                0,
        ...
        test('FIXED: handles "View Details" with bundle not found gracefully', async () => {
            ...
            mockRegistryManager.getBundleDetails
                .withArgs(bundleId)
                .rejects(new Error('Bundle not found'));
            ...
            assert.strictEqual(
                executeCommandSpy.callCount,
                1,
                'Should attempt to execute viewBundle command'
            );
    ...
    suite('Real-world Scenario - Amadeus Bundle', () => {
        test('FIXED: handles versioned bundle ID correctly', async () => {
            ...
            assert.strictEqual(
                mockRegistryManager.updateBundle.getCall(0).args[0],
                bundleId,
    ...
    suite('Command Registration Integration', () => {
        test('FIXED: promptRegistry.checkBundleUpdates command uses new behavior', async () => {
            ...
            assert.strictEqual(
                mockShowInformationMessage.callCount,
                2,
                'FIXED: Shows update dialog + success message'
            );
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                1,
                'FIXED: Updates only after user confirmation'
            );
        ...
        test('COMPARISON: old command behavior vs new command behavior', async () => {
            ...
            assert.strictEqual(
                mockRegistryManager.updateBundle.callCount,
                1,
                'BUG: Bundle was updated without showing check dialog'
            );
            assert.strictEqual(
                mockShowInformationMessage.callCount,
                1,
                'BUG: Only shows success message, no update dialog'
            );
