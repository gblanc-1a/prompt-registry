            ...
            fc.property(
                ...
                ({ owner, repo, versions }) => {
                    ...
                    const hasAllVersions =
                        consolidated[0].availableVersions.length === versions.length;
                    ...
                    fc.record({
                        ...
                        versions: fc.array(
                            fc
                                ...
                                .map(([major, minor, patch]) => `${major}.${minor}.${patch}`),
                            {
                                minLength: MIN_VERSIONS_PER_BUNDLE,
                                maxLength: MAX_VERSIONS_PER_BUNDLE,
                            }
                        ),
                    }),
                ...
                (repoConfigs) => {
                    ...
                    const allBundles = repoConfigs.flatMap((config) =>
                        config.versions.map((v) =>
                            BundleBuilder.github(config.owner, config.repo).withVersion(v).build()
                        )
                    );
                    ...
                    const allFromGitHub = consolidated.every((b) => b.sourceId === 'github-source');
