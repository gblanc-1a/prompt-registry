...
suite('GitHub Scaffold Validation Property-Based Tests', () => {
    ...
    const validCollectionIdGenerator = () => {
        ...
        return fc
            .array(fc.constantFrom(...chars), { minLength: 1, maxLength: 100 })
            .map((arr) => arr.join(''))
            .filter(
                (s) => !s.startsWith('-') && !s.endsWith('-') && !s.includes('--') && s.length > 0
            );
    };
    ...
    const invalidVersionGenerator = () => {
        return fc.oneof(
            ...
            // Invalid format
            fc.string({ minLength: 1, maxLength: 20 }).filter((s) => !/^\d+\.\d+\.\d+$/.test(s))
        );
    };
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (chatmodeKind, collectionId) => {
                    ...
                    assert.ok(
                        result.error.toLowerCase().includes('agent'),
                    ...
                    const collection = {
                        ...
                        items: [{ path: 'test/file.md', kind: chatmodeKind }],
                    };

                    const collectionResult = validateLib.validateCollectionObject(
                        collection,
                        'test.collection.yml'
                    );
    ...
    test('Property 8: Collection ID Validation', async function () {
        ...
        await fc.assert(
            fc.asyncProperty(validCollectionIdGenerator(), async (validId) => {
                ...
                assert.strictEqual(result.valid, true, `Valid ID '${validId}' should be accepted`);
                assert.strictEqual(result.error, undefined, `Valid ID should not have error`);
            }),
            { numRuns: PropertyTestConfig.RUNS.STANDARD, ...PropertyTestConfig.FAST_CHECK_OPTIONS }
        );
        ...
        await fc.assert(
            fc.asyncProperty(invalidCollectionIdGenerator(), async (invalidId) => {
                ...
                assert.ok(result.error, `Invalid ID should have error message`);
            }),
            { numRuns: PropertyTestConfig.RUNS.STANDARD, ...PropertyTestConfig.FAST_CHECK_OPTIONS }
        );
            ...
            fc.asyncProperty(
                ...
                async (testId) => {
                    ...
                    const collection = {
                        id: testId,
                        name: 'Test Collection',
                        items: [{ path: 'test/file.md', kind: 'prompt' }],
                    };
                    const objectResult = validateLib.validateCollectionObject(
                        collection,
                        'test.yml'
                    );
                    ...
                    if (directResult.valid) {
                        const hasIdError = objectResult.errors.some((e: string) =>
                            e.toLowerCase().includes('collection id must')
                        );
                        assert.ok(
                            !hasIdError,
...
`;
                        ...
                        for (const filePath of nonExistentPaths) {
                            const mentionsFile = result.errors.some((e: string) =>
                                e.includes(filePath)
                            );
                            assert.ok(
                                mentionsFile,
...
`;
        ...
        for (const missingField of requiredFields) {
            ...
            try {
                ...
                const mentionsMissingField = result.errors.some((e: string) =>
                    e.toLowerCase().includes(missingField)
                );
                assert.ok(
                    mentionsMissingField,
...
`;
            ...
            fc.asyncProperty(
                ...
                async (collectionId, version, kind) => {
                    ...
                    try {
                        ...
                        const collection = collectionsLib.readCollection(
                            tempDir,
                            'collections/test.collection.yml'
                        );
                        assert.strictEqual(
                        ...
                        const itemPaths = collectionsLib.resolveCollectionItemPaths(
                            tempDir,
                            collection
                        );
                        assert.ok(Array.isArray(itemPaths), 'Item paths should be an array');
                        assert.strictEqual(itemPaths.length, 1, 'Should have one item path');
                        assert.strictEqual(
                            itemPaths[0],
                            'prompts/test.md',
                        ...
                        assert.ok(
                            expectedBundleId.startsWith(`${repoSlug}-`),
                            'Bundle ID should start with repo slug'
                        );
                        assert.ok(
                            expectedBundleId.includes(`-${collectionId}-v`),
                            'Bundle ID should contain collection ID'
                        );
                        assert.ok(
                            expectedBundleId.endsWith(`-v${version}`),
                        ...
                        assert.ok(
                            expectedManifestAsset.endsWith('deployment-manifest.yml'),
                            'Manifest asset should be deployment-manifest.yml'
                        );
                        assert.ok(
                            expectedZipAsset.endsWith('.bundle.zip'),
                            'ZIP asset should end with .bundle.zip'
                        );
                        assert.ok(
                            expectedZipAsset.includes(collectionId),
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (repoSlug, collectionId, version) => {
                    ...
                    assert.ok(
                        bundleIdPattern.test(expectedBundleId),
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (collectionId, version) => {
                    ...
                    assert.ok(
                        prefixPart.endsWith(collectionId),
    ...
    test('Property 7: Version Computation from Manifest', async function () {
        ...
        const defaultVersionResult = validateLib.validateVersion();
        assert.strictEqual(
            defaultVersionResult.valid,
            true,
            'Undefined version should be valid (defaults to 1.0.0)'
        );
        assert.strictEqual(
            defaultVersionResult.normalized,
            '1.0.0',
        ...
        assert.strictEqual(
            nullVersionResult.valid,
            true,
            'Null version should be valid (defaults to 1.0.0)'
        );
        assert.strictEqual(
            nullVersionResult.normalized,
            '1.0.0',
        ...
        await fc.assert(
            fc.asyncProperty(invalidVersionGenerator(), async (invalidVersion) => {
                ...
                assert.ok(result.error, `Invalid version should have error message`);
                assert.ok(
                    result.error.toLowerCase().includes('x.y.z') ||
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async (collectionId, version) => {
                    ...
                    const hasVersionError = resultWithVersion.errors.some((e: string) =>
    ...
    test('Property 21: Initial Commit Publishes All Collections', async function () {
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                fc
                    .array(validCollectionIdGenerator(), { minLength: 1, maxLength: 5 })
                ...
                async (collectionIds) => {
                    ...
                    try {
                        ...
                        for (const collection of allCollections) {
                            ...
                            assert.ok(
                                collection.file.endsWith('.collection.yml'),
