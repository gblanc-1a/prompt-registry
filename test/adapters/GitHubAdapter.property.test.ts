...
suite('GitHubAdapter Property-Based Tests', () => {
            ...
            fc.asyncProperty(
                ...
                async (config: {
                    ...
                    try {
                        ...
                        const getSessionStub = iterationSandbox
                        ...
                        if (config.hasExplicitToken && config.explicitToken.trim().length > 0) {
                            ...
                            if (token !== config.explicitToken.trim() || method !== 'explicit') {
                                console.log(
                                    `Priority test failed: Expected explicit token, got method=${method}`
                                );
                            ...
                            if (getSessionStub.called || execStub.called) {
                                console.log(
                                    'Priority test failed: Other auth methods called when explicit token available'
                                );
                                assert.fail(
                                    'VSCode/gh CLI should not be attempted when explicit token is available'
                                );
                            }
                            ...
                            if (token !== config.vscodeToken || method !== 'vscode') {
                                console.log(
                                    `Priority test failed: Expected VSCode token, got method=${method}`
                                );
                            ...
                            if (execStub.called) {
                                console.log(
                                    'Priority test failed: gh CLI called when VSCode auth succeeded'
                                );
                                assert.fail(
                                    'gh CLI should not be attempted when VSCode auth succeeds'
                                );
                            }
                            ...
                            if (token !== config.ghCliToken.trim() || method !== 'gh-cli') {
                                console.log(
                                    `Priority test failed: Expected gh CLI token, got method=${method}`
                                );
                            ...
                            if (token === undefined && method !== 'none') {
                                console.log(
                                    `Priority test failed: Expected method=none, got method=${method}`
                                );
                                assert.strictEqual(method, 'none');
                            } else if (
                                token !== undefined &&
                                !['vscode', 'gh-cli'].includes(method)
                            ) {
                                console.log(
                                    `Priority test failed: Unexpected method=${method} with token present`
                                );
                                assert.fail(
                                    'Auth method should be vscode or gh-cli when token exists'
                                );
                            }
                        }
                    } finally {
    ...
    test('Whitespace-only explicit token is ignored', async () => {
        ...
        assert.notStrictEqual(
            method,
            'explicit',
            'Should not use whitespace-only token as explicit'
        );

        // Will fall back to VSCode or gh CLI or none depending on environment
        assert.ok(
            ['vscode', 'gh-cli', 'none'].includes(method),
            ...
            fc.asyncProperty(
                ...
                async (config) => {
                    ...
                    assert.ok(
                        ...
                        'Method should be tracked after invalidation'
                    );
                }
            ),
            { numRuns: TEST_CONFIG.RUNS.QUICK, ...TEST_CONFIG.FAST_CHECK_OPTIONS }
            ...
            fc.asyncProperty(
                ...
                async (config) => {
                    ...
                    try {
                        ...
                        const loggerCalled =
                            loggerStub.debug.called ||
                            loggerStub.info.called ||
                            loggerStub.warn.called;

                        if (!loggerCalled) {
                            console.log(
                                'Invalidation logging test failed: No logger calls detected'
                            );
                        ...
                        if (!loggerHelpers.hasLogContaining('invalidat')) {
                            console.log(
                                'Invalidation logging test failed: No log message contains "invalidat"'
                            );
            ...
            fc.asyncProperty(
                fc.record({
                    ...
                    htmlTemplate: fc.constantFrom(
                        ...
                        (text: string) =>
                            `<!DOCTYPE html><html><head><title>Error</title></head><body>${text}</body></html>`
                    ),
                }),
                async (config) => {
                    const iterationSandbox = sinon.createSandbox();
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!hasExtractedText) {
                                console.log(
                                    `Note: HTML text extraction didn't capture: "${config.errorText.substring(0, 30)}..."`
                                );
                            }
                        }
                    } finally {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (ErrorCheckers.isJsonParseError(err)) {
                                console.log(
                                    `HTML messaging test failed: Error is about JSON parsing, not auth: ${err.message}`
                                );
                                assert.fail(
                                    'Error should indicate authentication issue, not JSON parsing'
                                );
                            }
                            ...
                            if (!ErrorCheckers.indicatesAuthIssue(err)) {
                                console.log(
                                    `HTML messaging test: Error doesn't clearly indicate auth issue: ${err.message}`
                                );
                            }
                        }
                    } finally {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (errorCalls.length === 0) {
                                console.log(
                                    'Comprehensive logging test failed: No error logs captured'
                                );
                            ...
                            const hasAuthMethodLog = errorCalls.some((call) => {
                                ...
                                return (
                                    ...
                                    (message.includes('method') ||
                                        ...
                                        message.includes('vscode') ||
                                        message.includes('gh-cli'))
                                );
                            });

                            if (!hasAuthMethodLog) {
                                console.log(
                                    'Comprehensive logging test: No log mentions auth method'
                                );
                            }
                            ...
                            const hasUrlLog = errorCalls.some((call) => {
                                ...
                                return (
                                    message.includes('URL') || message.includes('api.github.com')
                                );
                            });
                ...
                async (config) => {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (check) {
                                const hasSuggestion = check.keywords.some((keyword) =>
                                    errorMsg.includes(keyword)
                                );
                                if (!hasSuggestion) {
                                    console.log(
                                        `Error-specific suggestions test: ${config.statusCode} error missing ${check.description}: ${err.message}`
                                    );
                                }
                            }

                            assert.ok(
                                errorMsg.includes(config.statusCode.toString()),
            ...
            fc.asyncProperty(
                ...
                async (config) => {
                    ...
                    try {
                        ...
                        iterationSandbox.stub(vscode.authentication, 'getSession').resolves();
                        ...
                        try {
                            ...
                            const mentionsAttemptedMethods =
                                errorMsg.includes('attempted') ||
                                errorMsg.includes('tried') ||
                                errorMsg.includes('method');

                            if (!mentionsAttemptedMethods) {
                                console.log(
                                    `Exhaustion listing test: Error doesn't mention attempted methods: ${err.message}`
                                );
                            }
                            ...
                            if (!mentionsExplicit) {
                                console.log(
                                    `Exhaustion listing test: Error doesn't mention explicit method: ${err.message}`
                                );
                            }
