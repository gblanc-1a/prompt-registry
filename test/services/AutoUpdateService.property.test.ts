...
import { InstalledBundle, Bundle, RegistrySource, SourceType } from '../../src/types/registry';
...
suite('AutoUpdateService - Property Tests', () => {
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bundleId, currentVersion, latestVersion) => {
                        ...
                        const [calledBundleId, calledVersion] =
                            mockRegistryManager.updateBundle.firstCall.args;
                        assert.strictEqual(calledBundleId, bundleId);
                        assert.strictEqual(calledVersion, latestVersion);
                        assert.strictEqual(
                            mockBundleNotifications.showAutoUpdateComplete.callCount,
                            1
                        );
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bundleId, version, errorMessage) => {
                        ...
                        const [calledBundleId, calledError] =
                            mockBundleNotifications.showUpdateFailure.firstCall.args;
                        assert.strictEqual(calledBundleId, bundleId);
                        assert.ok(
                            calledError.includes('Update failed') ||
                                calledError.includes('Rollback failed')
                        );
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bundleId, oldVersion, newVersion) => {
                        ...
                        const rollbackCall = mockRegistryManager.updateBundle
                            .getCalls()
                            .find(
                                (call) => call.args[0] === bundleId && call.args[1] === oldVersion
                            );
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bundleId1, bundleId2, version) => {
                        ...
                        const bundle1 = createMockInstalledBundle(bundleId1, '1.0.0', {
                            installPath: '/mock/path1',
                        });
                        const bundle2 = createMockInstalledBundle(bundleId2, '1.0.0', {
                            installPath: '/mock/path2',
                        });
                        const updatedBundle1 = createMockInstalledBundle(bundleId1, version, {
                            installPath: '/mock/path1',
                        });
                        const updatedBundle2 = createMockInstalledBundle(bundleId2, version, {
                            installPath: '/mock/path2',
                        });
                        ...
                        mockRegistryManager.listInstalledBundles
                            ...
                            .onCall(3)
                            .resolves([updatedBundle1, updatedBundle2]);
    ...
    suite('Property 22: Batch update controlled concurrency processing', () => {
        test('should process updates in batches with controlled concurrency', async () => {
            await fc.assert(
                fc.asyncProperty(fc.integer({ min: 1, max: 10 }), async (numUpdates) => {
    ...
    suite('Property 23: Batch update summary display', () => {
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (numUpdates, failureFlags) => {
                        ...
                        const mappedFailureFlags = updates.map(
                            (_, i) => failureFlags[i % failureFlags.length]
                        );
        ...
        test('should show summary even when all updates fail', async () => {
            await fc.assert(
                fc.asyncProperty(fc.integer({ min: 1, max: 5 }), async (numUpdates) => {
                    const updates = Array.from({ length: numUpdates }, (_, i) =>
                        createUniqueUpdateCheckResult(i)
                    );
                    resetAllMocks();
                ...
                fc.asyncProperty(
                    ...
                    async (numUpdates) => {
                        ...
                        for (const u of updates) {
                            bundleVersions.set(u.bundleId, u.currentVersion);
                        }
                        ...
                        mockRegistryManager.updateBundle.callsFake(
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    async (bundleId, currentVersion, latestVersion, sourceType) => {
                        ...
                        mockRegistryManager.getBundleDetails
                            .withArgs(bundleId)
                            .resolves(mockBundle);
