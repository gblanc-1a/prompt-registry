...
suite('Compute Collection Version Tests', () => {
    ...
    test('version uses collection.version if greater than last tag, else patch bumps', function () {
        ...
        fs.writeFileSync(
            collectionFile,
            ['id: a', 'name: A', 'version: "1.0.0"', 'items: []'].join('\n')
        );
        ...
        let res = run(
            'node',
            [versionScript, '--collection-file', 'collections/a.collection.yml'],
            root,
            env
        );
        ...
        assert.strictEqual(
            out1.nextVersion,
            '1.0.0',
            'Without tags, should use collection.version'
        );
        ...
        assert.strictEqual(
            out2.nextVersion,
            '1.0.1',
            'When version <= last tag, should patch bump'
        );

        // Manual major bump should be respected
        fs.writeFileSync(
            collectionFile,
            ['id: a', 'name: A', 'version: "2.0.0"', 'items: []'].join('\n')
        );
    ...
    test('defaults to 1.0.0 when version field is missing', function () {
        ...
        fs.mkdirSync(path.dirname(collectionFile), { recursive: true });
        fs.writeFileSync(collectionFile, ['id: b', 'name: B', 'items: []'].join('\n'));
        ...
        const res = run(
            'node',
            [versionScript, '--collection-file', 'collections/b.collection.yml'],
            root,
            env
        );
    ...
    test('generates correct tag format', function () {
        ...
        fs.writeFileSync(
            collectionFile,
            ['id: my-collection', 'name: My Collection', 'version: "1.2.3"', 'items: []'].join('\n')
        );
        ...
        const res = run(
            'node',
            [versionScript, '--collection-file', 'collections/my-collection.collection.yml'],
            root,
            env
        );
        ...
        assert.strictEqual(
            out.tag,
            'my-collection-v1.2.3',
            'Tag should follow {collection-id}-v{version} format'
        );
    });
    ...
    test('fails for invalid semver version', function () {
        ...
        fs.writeFileSync(
            collectionFile,
            ['id: c', 'name: C', 'version: "invalid"', 'items: []'].join('\n')
        );
        ...
        const res = run(
            'node',
            [versionScript, '--collection-file', 'collections/c.collection.yml'],
            root,
            env
        );
        ...
        assert.ok(
            res.stderr.toLowerCase().includes('semver') ||
                res.stderr.toLowerCase().includes('version'),
