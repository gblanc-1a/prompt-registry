...
import { BundleUpdate } from '../../src/types/registry';
            ...
            fc.asyncProperty(
                ...
                fc.uniqueArray(
                    fc.record({
                        ...
                        installedVersion: BundleGenerators.versionTuple(
                            MAX_MAJOR_VERSION,
                            MAX_MINOR_VERSION,
                            MAX_PATCH_VERSION
                        ),
                        latestVersion: BundleGenerators.versionTuple(
                            MAX_MAJOR_VERSION,
                            MAX_MINOR_VERSION,
                            MAX_PATCH_VERSION
                        ),
                ...
                async (bundleConfigs) => {
                    ...
                    for (const result of results) {
                        const expected = expectedUpdates.find(
                            (e) => e.bundleId === result.bundleId
                        );
                        if (!expected) {
                            return false;
                        }
                        ...
                        const [cMajor, cMinor, cPatch] = result.currentVersion
                            .split('.')
                            .map(Number);
                        const [lMajor, lMinor, lPatch] = result.latestVersion
                            .split('.')
                            .map(Number);
                    ...
                    try {
                        ...
                        for (let i = 0; i < subsequentCalls; i++) {
                            ...
                            for (const [j, cachedResult] of cachedResults.entries()) {
                                ...
                                if (
                                    cachedResult.currentVersion !== firstResults[j].currentVersion
                                ) {
                                    return false;
                                }
                    ...
                    try {
                        ...
                        if (hasNewUpdates && bundles.length > 0) {
                            const newUpdates: BundleUpdate[] = [
                                ...
                                {
                                    bundleId:
                                        'new-bundle-' + Math.random().toString(36).substring(7),
                            ...
                            testRegistryManager.getBundleDetails
                                .withArgs(newUpdates.at(-1)!.bundleId)
                                .resolves({
                ...
                async ({ githubSources, nonGithubSources }) => {
                    ...
                    try {
                        ...
                        for (const githubSource of githubSources) {
                            const wasCalledForGithub = testRegistryManager.syncSource.calledWith(
                                githubSource.id
                            );
                        ...
                        for (const nonGithubSource of nonGithubSources) {
                            const wasCalledForNonGithub = testRegistryManager.syncSource.calledWith(
                                nonGithubSource.id
                            );
