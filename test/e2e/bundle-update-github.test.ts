...
`;
    ...
    function setupReleaseMocks(
        ...
        const releasesResponse = releases.map((r, index) => ({
            ...
            published_at: new Date(Date.now() - index * 86_400_000).toISOString(),
            assets: [
                {
        ...
        nock('https://api.github.com').persist().get('/repos/test-owner/test-repo').reply(200, {
            name: 'test-repo',
    ...
    suite('GitHub Bundle Update Workflow', () => {
        test('Example 2.1: Specific version installation records correct version', async function () {
            ...
            const v1Bundle = rawBundles.find(
                (b) => b.id === 'test-owner-test-repo-test-collection-1.0.0'
            );
            ...
            await testContext.registryManager.installBundle(v1Bundle.id, {
                scope: 'user',
                version: '1.0.0',
            });
            ...
            assert.strictEqual(
                installed[0].version,
                '1.0.0',
                'Installation record should show v1.0.0'
            );
        });
        ...
        test('Example 2.4: Update action installs latest version', async function () {
            ...
            assert.strictEqual(
                installedAfter[0].version,
                '2.0.0',
                'Should have v2.0.0 installed after update'
            );
        });
        ...
        test('Example 2.5: Previous installation record is removed', async function () {
            ...
            const installedAfter = await testContext.registryManager.listInstalledBundles();
            const v1Record = installedAfter.find(
                (b) => b.bundleId === v1BundleId && b.version === '1.0.0'
            );
            assert.ok(!v1Record, 'v1.0.0 installation record should be removed');

            // Verify only v2.0.0 exists
            assert.strictEqual(
                installedAfter.length,
                1,
                'Should have exactly one installation record'
            );
        ...
        test('Example 2.6: Previous version files are removed', async function () {
            ...
            if (v1InstallPath !== v2InstallPath) {
                assert.ok(
                    !fs.existsSync(v1InstallPath),
                    'v1.0.0 install directory should be removed'
                );
            }
            ...
            assert.ok(
                !v2Content.includes('INITIAL_CONTENT'),
                'v2.0.0 should not have initial content'
            );
        });
