...
console.log('Running eslint to find no-console violations...');
let lintOutput;
try {
...
// Parse the output to find no-console violations
...
for (const line of lines) {
    ...
    } else if (currentFile && line.includes('no-console')) {
        // Extract line number
...
console.log(`Found ${violations.length} no-console violations`);
...
for (const [filePath, lineNumbers] of Object.entries(fileViolations)) {
    ...
    for (const lineNumber of lineNumbers) {
        ...
        const comment = `${indent}// eslint-disable-next-line no-console -- TODO: Migrate to Logger (Req 6)`;
