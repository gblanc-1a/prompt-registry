...
`;
            ...
            .reply(200, [
                {
                    ...
                    type: 'file',
                    download_url:
                        'https://raw.githubusercontent.com/test-owner/awesome-copilot-test/main/collections/test-collection.collection.yml',
                },
            ]);
    ...
    function setupValidationMocks(): void {
        nock('https://api.github.com')
            .get('/repos/test-owner/awesome-copilot-test/contents/collections')
            .query({ ref: 'main' })
            .reply(200, [{ name: 'test-collection.collection.yml', type: 'file' }]);
    }
    ...
    suite('Awesome Copilot Update Workflow', () => {
        test('Example 1.1: Update command downloads from configured branch', async function () {
            ...
            nock('https://raw.githubusercontent.com')
                .persist()
                .get(
                    '/test-owner/awesome-copilot-test/main/collections/test-collection.collection.yml'
                )
        ...
        test('Example 1.2: Bundle files are replaced after update', async function () {
            ...
            if (fs.existsSync(promptPath)) {
                ...
                assert.ok(
                    initialContent.includes('INITIAL_CONTENT_MARKER'),
            ...
            if (fs.existsSync(newPromptPath)) {
                ...
                assert.ok(
                    updatedContent.includes('UPDATED_CONTENT_MARKER'),
                    'Updated content should contain new marker'
                );
                assert.ok(
                    !updatedContent.includes('INITIAL_CONTENT_MARKER'),
        ...
        test('Example 1.4: Installation scope is preserved', async function () {
            ...
            await testContext.registryManager.installBundle(bundleToInstall!.id, {
                scope: installScope,
            });
