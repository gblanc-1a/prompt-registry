...
import { createE2ETestContext, generateTestId, waitForCondition } from './e2eTestHelpers';
    ...
    suite('createE2ETestContext', () => {
        test('should create isolated storage directory for each test (Example 3.1)', async function () {
            ...
            try {
                ...
                assert.ok(fs.existsSync(paths1.installed), 'Installed directory should be created');
                assert.ok(fs.existsSync(paths1.cache), 'Cache directory should be created');
            } finally {
        ...
        test('should cleanup all test artifacts after teardown (Example 3.2)', async function () {
            ...
            assert.ok(fs.existsSync(storagePath), 'Storage directory should exist before cleanup');
            ...
            assert.ok(!fs.existsSync(testFile), 'Test artifacts should be removed after cleanup');
        });
