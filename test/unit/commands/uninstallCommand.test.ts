...
describe('UninstallCommand Test Suite', () => {
    ...
    describe('getInstalledFiles', () => {
        ...
        it('should aggregate files from all scopes correctly', async () => {
            ...
            mockInstallationManager.setMockInstallationInfo('user', {
                installedFiles: ['prompt-registry-core/file1.txt', 'prompt-registry-core/file2.txt'],
                scope: 'user',
            ...
            assert.strictEqual(result.some(f => f.scope === 'user' && f.path === 'prompt-registry-core/file1.txt'), true);
            ...
            const userFile1 = result.find(f => f.scope === 'user' && f.path === 'prompt-registry-core/file1.txt');
            const expectedPath = path.join('/home/user/.olaf', 'prompt-registry-core/file1.txt');
    ...
    describe('executeUninstallAll - Core Logic Tests', () => {
        it('should handle no installations correctly', async () => {
            ...
            assert.strictEqual(messageContent, 'Prompt Registry is not installed in any scope.');
            
            const logs = mockLogger.getLogs();
            assert.strictEqual(logs.some(log => log.message === 'No Prompt Registry installations found'), true);
        });
        ...
        it('should construct correct confirmation message', async () => {
            ...
            const expectedMessage = 'Are you sure you want to uninstall Prompt Registry from ALL scopes (user, workspace)? This will remove all recorded files and cannot be undone.';
    ...
    describe('showRemovalPreview', () => {
        it('should show message when no files are found', async () => {
            ...
            // Assert
            assert.strictEqual(informationMessageCalled, true);
            assert.strictEqual(messageContent, 'No Prompt Registry files found to remove.');
        });
        ...
        it('should display grouped file preview', async () => {
            ...
            // Assert
            assert.strictEqual(informationMessageCalled, true);
            assert.strictEqual(messageContent.includes('Prompt Registry Installation Preview'), true);
