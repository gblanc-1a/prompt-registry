...
export class OlafAdapter extends RepositoryAdapter {
    ...
    async fetchBundles(): Promise<Bundle[]> {
        ...
        try {
            ...
            this.logger.info(
                `[OlafAdapter] Found ${bundleDefinitions.length} bundle definitions in repository`
            );
            ...
            for (const bundleInfo of bundleDefinitions) {
                try {
                    ...
                    bundles.push(bundle);
                    this.logger.debug(
                        `[OlafAdapter] Created bundle: ${bundle.id} (${bundleInfo.validatedSkills.length} skills)`
                    );
                } catch (error) {
                    this.logger.warn(
                        `[OlafAdapter] Failed to create bundle from definition ${bundleInfo.fileName}: ${error}`
                    );
            ...
            this.logger.info(
                `[OlafAdapter] Successfully created ${bundles.length} bundles from definitions`
            );
            return bundles;
        } catch (error) {
    ...
    private createBundleFromDefinition(bundleInfo: BundleDefinitionInfo): Bundle {
        ...
        const skillNames = bundleInfo.validatedSkills
            .map((s) => s.manifest.name || s.folderName)
            .join(', ');
        ...
        this.logger.debug(
            `[OlafAdapter] Created bundle: ${bundle.id} (${bundle.name} v${bundle.version}, ${skillCount} skills)`
        );
        return bundle;
    }
        ...
        try {
            ...
            try {
                ...
                if (bundleCount === 0) {
                    ...
                    this.logger.info(
                        `[OlafAdapter] Found ${bundleCount} valid bundle definition(s)`
                    );
                }
            } catch (scanError) {
    ...
    private async scanBundleDefinitions(): Promise<BundleDefinitionInfo[]> {
        ...
        try {
            ...
            this.logger.debug(
                `[OlafAdapter] Found ${jsonFiles.length} JSON files in bundles/ directory`
            );
            ...
            for (const jsonFile of jsonFiles) {
                try {
                    ...
                    try {
                        ...
                        this.logger.info(
                            `[OlafAdapter] Successfully processed bundle: ${fileName} (${validatedSkills.length} skills)`
                        );
                    } catch (skillError) {
            ...
            if (bundleDefinitions.length > 0) {
                this.logger.info(
                    `[OlafAdapter] Successfully processed ${bundleDefinitions.length} bundle(s)`
                );
            }

            if (errors.length > 0) {
                this.logger.warn(
                    `[OlafAdapter] Encountered ${errors.length} error(s) while processing bundles`
                );
            }
     ...
     */
    private async parseBundleDefinition(
        jsonFile: GitHubDirectoryContent
        ...
        try {
            ...
            if (error instanceof SyntaxError) {
                throw new TypeError(`Invalid JSON syntax in ${jsonFile.name}: ${error.message}`);
            }
            throw error;
        }
    }
    ...
    private async validateSkillReferences(
        ...
        for (let i = 0; i < bundleDefinition.skills.length; i++) {
            ...
            try {
                ...
                this.logger.debug(
                    `[OlafAdapter] Validated ${skillContext}: ${files.length} files, ${manifest.entry_points?.length || 0} entry points`
                );
            } catch (error) {
     ...
     */
    private async parseLocalSkillManifest(
        manifestFile: GitHubDirectoryContent
    ): Promise<LocalOlafSkillManifest> {
    ...
    private async scanSkillsDirectory(): Promise<SkillInfo[]> {
        ...
        try {
            ...
            this.logger.debug(
                `[OlafAdapter] Found ${skillDirectories.length} potential skill directories`
            );
            ...
            for (const skillDir of skillDirectories) {
                try {
                    ...
                    if (skillInfo) {
                        ...
                        this.logger.debug(
                            `[OlafAdapter] Successfully processed skill: ${skillInfo.id}`
                        );
                    }
                } catch (error) {
                    this.logger.warn(
                        `[OlafAdapter] Failed to process skill directory ${skillDir.name}: ${error}`
                    );
    ...
    private async processSkillDirectory(
        skillDir: GitHubDirectoryContent,
        owner: string,
        repo: string
    ): Promise<SkillInfo | null> {
        ...
        try {
            ...
            const skillContents: GitHubDirectoryContent[] =
                await this.makeGitHubRequest(skillContentsUrl);

            // Look for skill-manifest.json
            const manifestFile = skillContents.find(
                (file) => file.name === 'skill-manifest.json' && file.type === 'file'
            );

            if (!manifestFile) {
                this.logger.warn(
                    `[OlafAdapter] Skill ${skillName} missing skill-manifest.json, skipping`
                );
                return null;
            }
     ...
     */
    private async parseSkillManifest(
        manifestUrl: string,
        ...
        try {
            ...
            this.logger.debug(
                `[OlafAdapter] Downloaded manifest content (${manifestContent.length} bytes): ${manifestText.substring(0, 200)}...`
            );
            ...
            const manifest: SkillManifest = {
                ...
                dependencies: Array.isArray(rawManifest.dependencies)
            ...
            this.logger.debug(
                `[OlafAdapter] Successfully parsed manifest for skill: ${manifest.name}`
            );
            return manifest;
        } catch (error) {
            this.logger.error(
                `[OlafAdapter] Failed to parse skill manifest from ${manifestUrl}: ${error}`
            );
            ...
            this.logger.warn(
                `[OlafAdapter] Using default manifest with name '${defaultManifest.name}' due to parse failure`
            );
    ...
    private createBundleFromSkill(skill: SkillInfo): Bundle {
        ...
        this.logger.debug(
            `[OlafAdapter] Created bundle: ${bundle.id} (${bundle.name} v${bundle.version})`
        );
        return bundle;
    }
    ...
    async downloadBundle(bundle: Bundle): Promise<Buffer> {
        ...
        try {
            ...
            this.logger.info(
                `[OlafAdapter] Found bundle definition: ${bundleInfo.fileName} with ${bundleInfo.validatedSkills.length} skill(s)`
            );
            ...
            this.logger.info(
                `[OlafAdapter] Successfully packaged bundle ${bundleFileName} (${zipBuffer.length} bytes)`
            );
            return zipBuffer;
        } catch (error) {
            this.logger.error(
                `[OlafAdapter] Failed to download bundle ${bundleFileName}: ${error}`
            );
    ...
    private async packageBundleAsZip(bundleInfo: BundleDefinitionInfo): Promise<Buffer> {
        ...
        try {
                ...
                for (const item of skillContents) {
                    if (item.type === 'file' && item.download_url) {
                        try {
                            ...
                            this.logger.debug(
                                `[OlafAdapter] Added file to ZIP: ${filePath} (${fileContent.length} bytes)`
                            );
                        } catch (error) {
                            this.logger.warn(
                                `[OlafAdapter] Failed to download file ${item.name}: ${error}`
                            );
            ...
            this.logger.debug(
                `[OlafAdapter] Created ZIP bundle for ${bundleInfo.fileName}: ${zipBuffer.length} bytes, ${bundleInfo.validatedSkills.length} skill(s)`
            );
            return zipBuffer;
        } catch (error) {
            this.logger.error(
                `[OlafAdapter] Failed to package bundle ${bundleInfo.fileName}: ${error}`
            );
    ...
    private generateBundleDeploymentManifest(bundleInfo: BundleDefinitionInfo): any {
        ...
        const deploymentManifest = {
            ...
            metadata: {
                ...
                license: 'Unknown',
                keywords: [...(definition.metadata.tags || []), 'olaf', 'bundle', 'skills'],
            },
        ...
        this.logger.debug(
            `[OlafAdapter] Generated deployment manifest for bundle: ${bundleInfo.id} with ${validatedSkills.length} skill(s)`
        );
        return deploymentManifest;
    }
    ...
    private async ensureRuntimeInstalled(): Promise<void> {
        try {
            this.logger.info(
                '[OlafAdapter] Ensuring OLAF runtime is installed (required for OLAF skills)'
            );
            ...
            const runtimeInstalled =
                await this.runtimeManager.ensureRuntimeInstalled(workspacePath);
            ...
            if (workspacePath) {
                ...
                this.logger.warn(
                    '[OlafAdapter] No workspace detected, skipping symbolic link creation'
                );
            }
            ...
            if (error instanceof Error) {
                if (error.message.includes('network') || error.message.includes('download')) {
                    throw new Error(
                        'Failed to download OLAF runtime. Please check your internet connection and try again.'
                    );
                } else if (
                    ...
                    throw new Error(
                        'Permission denied while installing OLAF runtime. Please check file permissions.'
                    );
                    ...
                    throw new Error(
                        'Failed to access OLAF runtime repository. The repository may not exist or may be private.'
                    );
                } else {
                    throw new Error(`OLAF runtime installation failed: ${error.message}`);
                }
            } else {
                throw new TypeError('OLAF runtime installation failed with unknown error');
            }
        }
    }
    ...
    private async packageSkillAsBundle(skill: SkillInfo): Promise<Buffer> {
        ...
        try {
            ...
            this.logger.debug(
                `[OlafAdapter] Created ZIP bundle for ${skill.folderName}: ${zipBuffer.length} bytes`
            );
            return zipBuffer;
        } catch (error) {
            this.logger.error(
                `[OlafAdapter] Failed to package skill ${skill.folderName}: ${error}`
            );
    ...
    private async addDirectoryToZip(
        ...
        try {
            ...
            const dirContents: GitHubDirectoryContent[] =
                await this.makeGitHubRequest(dirContentsUrl);
            ...
            for (const item of dirContents) {
                if (item.type === 'file' && item.download_url) {
                    try {
                        ...
                        this.logger.warn(
                            `[OlafAdapter] Failed to download nested file ${item.name}: ${error}`
                        );
                    }
                } else if (item.type === 'dir') {
    ...
    getManifestUrl(_bundleId: string, _version?: string): string {
        const { owner, repo } = this.parseGitHubUrl();
        // Extract bundle file name from bundle ID (format: olaf-{owner}-{repo}-{bundleFileName})
        const bundleFileName = _bundleId.replace(`olaf-${owner}-${repo}-`, '');
    ...
    private async registerBundleInCompetencyIndex(
        ...
        this.logger.info(
            `[OlafAdapter] Registering bundle skills in competency index: ${bundleId}`
        );
        ...
        try {
            ...
            if (!workspaceFolders || workspaceFolders.length === 0) {
                this.logger.warn(
                    '[OlafAdapter] No workspace found, skipping competency index registration'
                );
                return;
            }
            ...
            const firstFolder = workspaceFolders[0];
            if (!firstFolder) {
                this.logger.warn('[OlafAdapter] No workspace folder available');
                return;
            }
            ...
            if (!fs.existsSync(competencyIndexDir)) {
                this.logger.info(
                    `[OlafAdapter] Directory does not exist, creating: ${competencyIndexDir}`
                );
            ...
            if (fs.existsSync(competencyIndexPath)) {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    this.logger.info(
                        `[OlafAdapter] Found ${competencyIndex.length} existing skills`
                    );
                    ...
                    this.logger.info(
                        `[OlafAdapter] Found legacy format, migrating ${competencyIndex.length} existing skills`
                    );
                } else {
                    this.logger.warn(
                        `[OlafAdapter] Invalid competency index format, creating new array`
                    );
                    competencyIndex = [];
                }
            } else {
                this.logger.info(
                    `[OlafAdapter] Competency index does not exist, will create new one`
                );
            }
            ...
            if (!bundleInfo) {
                this.logger.warn(
                    `[OlafAdapter] Bundle definition not found for ${bundleId}, skipping competency index registration`
                );
                return;
            }
            ...
            this.logger.info(
                `[OlafAdapter] Writing updated competency index with ${competencyIndex.length} skills`
            );
            ...
            if (fs.existsSync(competencyIndexPath)) {
                ...
                if (Array.isArray(verifyIndex)) {
                    this.logger.info(
                        `[OlafAdapter] Competency index verification successful: ${verifyIndex.length} total skills`
                    );
                } else {
                    this.logger.error(
                        `[OlafAdapter] Competency index verification failed: invalid format`
                    );
                }
            } else {
            ...
            this.logger.info(
                `[OlafAdapter] Successfully registered ${bundleInfo.validatedSkills.length} skills in competency index`
            );
        } catch (error) {
            this.logger.error(
                `[OlafAdapter] Failed to register bundle in competency index: ${error}`
            );
            if (error instanceof Error) {
     ...
     */
    private async registerSkillEntryPoints(
        skill: SkillInfo,
        sourceName: string,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- TODO: Add proper types (Req 7)
        competencyIndex: any[]
        ...
        try {
            ...
            if (entryPoints.length === 0) {
                this.logger.warn(
                    `[OlafAdapter] Skill ${skill.id} has no entry points, skipping competency index registration`
                );
                return;
            }
            ...
            for (const entryPoint of entryPoints) {
                ...
                this.logger.info(
                    `[OlafAdapter] Processing entry point for skill ${skill.id}: ${promptFilePath}`
                );
                ...
                if (existingIndex !== -1) {
                    ...
                    this.logger.info(
                        `[OlafAdapter] Updating existing skill entry in competency index: ${skillEntry.file}`
                    );
                    competencyIndex[existingIndex] = skillEntry;
                } else {
                    // Add new entry
                    this.logger.info(
                        `[OlafAdapter] Adding new skill entry to competency index: ${skillEntry.file}`
                    );
            ...
            this.logger.error(
                `[OlafAdapter] Failed to register skill ${skill.id} entry points: ${error}`
            );
            throw error;
        }
    }
    ...
    private async unregisterBundleFromCompetencyIndex(
        ...
        try {
            ...
            if (!workspaceFolders || workspaceFolders.length === 0) {
                this.logger.warn(
                    '[OlafAdapter] No workspace found, skipping competency index unregistration'
                );
                return;
            }
            ...
            if (!fs.existsSync(competencyIndexPath)) {
                this.logger.info(
                    `[OlafAdapter] Competency index does not exist, nothing to unregister`
                );
                return;
            }
            ...
            if (Array.isArray(parsed)) {
                ...
                competencyIndex = parsed.skills;
                this.logger.info(
                    `[OlafAdapter] Found legacy format with ${competencyIndex.length} existing skills`
                );
            } else {
                this.logger.warn(
                    `[OlafAdapter] Invalid competency index format, nothing to unregister`
                );
                return;
            }
            ...
            try {
                ...
                this.logger.warn(
                    `[OlafAdapter] Could not scan bundle definitions during uninstall: ${error}`
                );
            }
            ...
            if (initialLength > finalLength) {
                this.logger.info(
                    `[OlafAdapter] Removed ${initialLength - finalLength} skill(s) from competency index`
                );
                ...
                if (fs.existsSync(competencyIndexPath)) {
                    ...
                    if (Array.isArray(verifyIndex)) {
                        this.logger.info(
                            `[OlafAdapter] Verification: File exists and contains ${verifyIndex.length} skills`
                        );
                    }
                } else {
                    this.logger.error(
                        `[OlafAdapter] Verification: File does not exist after write!`
                    );
                }
            } else {
            ...
            this.logger.error(
                `[OlafAdapter] Failed to unregister bundle from competency index: ${error}`
            );
            if (error instanceof Error) {
                this.logger.error(`[OlafAdapter] Error stack: ${error.stack}`);
            }
            ...
            https
                ...
                .get(url, { headers }, (res: any) => {
                    ...
                    res.on('end', () => {
                        if (res.statusCode >= 400) {
                            this.logger.error(
                                `[OlafAdapter] HTTP ${res.statusCode}: ${res.statusMessage}`
                            );
                        ...
                        try {
                            ...
                            this.logger.error(
                                `[OlafAdapter] Failed to parse JSON response: ${error}`
                            );
                            this.logger.error(
                                `[OlafAdapter] Response preview: ${data.substring(0, 200)}`
                            );
