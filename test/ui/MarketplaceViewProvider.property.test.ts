...
suite('MarketplaceViewProvider - Property Tests', () => {
    suite('Property 4: Button state correctness', () => {
        ...
        test('should handle version prefixes correctly', () => {
            ...
            const versionWithPrefixArbitrary = fc
                .tuple(
                    fc.integer({ min: 0, max: 10 }),
                    fc.integer({ min: 0, max: 20 }),
                    fc.integer({ min: 0, max: 50 }),
        ...
        test('should always return "install" when no version is installed', () => {
            ...
            fc.assert(
                fc.property(semverArbitrary, (latestVersion) => {
            ...
            fc.assert(
                fc.property(
                    ...
                    ({ installedVersion, latestVersion }) => {
                        ...
                        const updateAvailable = VersionManager.isUpdateAvailable(
                            installedVersion,
                            latestVersion
                        );
                        ...
                        const stillNeedsUpdate = VersionManager.isUpdateAvailable(
                            newInstalledVersion,
                            latestVersion
                        );
            ...
            fc.assert(
                fc.property(
                    ...
                    ([major, minor, patch, gap]) => {
                        ...
                        const cmp = VersionManager.compareVersions(
                            newInstalledVersion,
                            latestVersion
                        );
