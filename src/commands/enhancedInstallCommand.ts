...
/**
 * Enhanced command handler for installing Prompt Registry components with integrity verification
 */
export class EnhancedInstallCommand {
    ...
    public async execute(): Promise<void> {
        try {
            this.logger.info('Starting enhanced Prompt Registry installation...');
            ...
            await vscode.window.withProgress({
                ...
                title: 'Enhanced Prompt Registry Installation',
                cancellable: false
            }, async (progress) => {
    ...
    private async handleExistingInstallation(existingInstallation: any, scope: InstallationScope): Promise<boolean> {
        ...
        const action = await vscode.window.showWarningMessage(
            `Existing ${isEnhanced ? 'enhanced' : 'legacy'} Prompt Registry installation found (version: ${version}). What would you like to do?`,
            { modal: true },
            'Upgrade',
            'Reinstall', 
            'Cancel'
        ...
        try {
            ...
            try {
                ...
                const viewReport = await vscode.window.showInformationMessage(
                    `Prompt Registry v${selectedVersion.version} installed successfully with enhanced integrity tracking!`,
                    'View Details'
                );
    ...
    private async showDetailedReport(installResult: any): Promise<void> {
        ...
        const details = [
            '# Enhanced Prompt Registry Installation Report',
            '',
