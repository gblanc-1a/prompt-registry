...
import {
    ...
    ValidationError,
    ValidationWarning,
} from '../../src/utils/collectionValidator';
    ...
    suite('validateCollection', () => {
        ...
        suite('Required Fields', () => {
            test('should fail if id is missing', () => {
                ...
                assert.ok(
                    result.errors.some((e: ValidationError) => e.message.includes('id')),
            ...
            test('should fail if name is missing', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/missing-name.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);

                assert.strictEqual(result.valid, false, 'Collection should be invalid');
                assert.ok(
                    result.errors.some((e: ValidationError) => e.message.includes('name')),
            ...
            test('should fail if description is missing', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/missing-description.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);

                assert.strictEqual(result.valid, false, 'Collection should be invalid');
                assert.ok(
                    result.errors.some((e: ValidationError) => e.message.includes('description')),
            ...
            test('should fail if items is missing', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/missing-items.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);

                assert.strictEqual(result.valid, false, 'Collection should be invalid');
                assert.ok(
                    result.errors.some((e: ValidationError) => e.message.includes('items')),
                    'Should have error about missing items'
                );
            });
        });
        ...
        suite('ID Validation', () => {
            test('should fail if id has uppercase letters', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/invalid-id-uppercase.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
            ...
            test('should fail if id has spaces', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/invalid-id-spaces.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
            ...
            test('should accept valid id with lowercase, numbers, and hyphens', () => {
                ...
                assert.strictEqual(result.valid, true, 'Collection should be valid');
                const idErrors = result.errors.filter((e: ValidationError) =>
                    e.message.toLowerCase().includes('id')
                );
        ...
        suite('Description Validation', () => {
            test('should warn if description exceeds 500 characters', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'warnings/long-description.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
        ...
        suite('Items Validation', () => {
            test('should fail if item is missing path field', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/item-missing-path.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
            ...
            test('should fail if item is missing kind field', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/item-missing-kind.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
                ...
                assert.ok(
                    result.errors.some((e: ValidationError) =>
                        e.message.toLowerCase().includes('kind')
                    ),
            ...
            test('should fail if kind is invalid', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/invalid-kind.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);

                assert.strictEqual(result.valid, false, 'Collection should be invalid');
                assert.ok(
                    result.errors.some((e) => e.message.toLowerCase().includes('kind')),
            ...
            test('should accept valid kinds: prompt, instruction, chat-mode, agent', () => {
                ...
                assert.strictEqual(result.valid, true, 'Collection should be valid');
                const kindErrors = result.errors.filter((e: ValidationError) =>
                    e.message.toLowerCase().includes('kind')
                );
        ...
        suite('File Reference Validation', () => {
            test('should fail if referenced file does not exist', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/missing-file-ref.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
        ...
        suite('Tags Validation', () => {
            test('should warn if more than 10 tags', () => {
                ...
                assert.ok(
                    result.warnings.some(
                        (w: ValidationWarning) =>
                            w.message.includes('tag') && w.message.includes('10')
                    ),
            ...
            test('should accept collections with valid tags', () => {
                ...
                assert.strictEqual(result.valid, true, 'Collection should be valid');
                const tagErrors = result.errors.filter((e: ValidationError) =>
        ...
        suite('YAML Parsing', () => {
            test('should fail gracefully with YAML syntax errors', () => {
                const collectionPath = path.join(
                    fixturesDir,
                    'invalid/yaml-syntax-error.collection.yml'
                );
                const result = validator.validateCollection(collectionPath, fixturesDir);
