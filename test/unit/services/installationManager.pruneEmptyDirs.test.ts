...
describe('InstallationManager.pruneEmptyDirs Test Suite', () => {
    ...
    describe('pruneEmptyDirs - Directory Structure Cleanup', () => {
        ...
        it('should not remove directories that contain other files', async () => {
            ...
            // Create metadata file
            const metadata = {
                installedFiles: ['prompt-registry-core/deep/file.txt'], // Only one file was installed
                extractionPath: extractionPath,
                scope: 'user',
                version: '1.0.0',
                platform: 'vscode'
            ...
            assert.strictEqual(fs.existsSync(path.join(extractionPath, 'prompt-registry-core')), true, 'prompt-registry-core directory should remain (has remaining file)');
