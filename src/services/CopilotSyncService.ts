...
export class CopilotSyncService {
    ...
    private getCopilotPromptsDirectory(): string {
        ...
        if (userIndex === -1) {
            ...
            const profilesMatch = baseDir.match(
                new RegExp(`profiles${escapedSep}([^${escapedSep}]+)`)
            );
            if (profilesMatch) {
        ...
        const escapedSep = escapeRegex(path.sep);
        const profilesMatch = remainingPath.match(
            new RegExp(`^${escapedSep}profiles${escapedSep}([^${escapedSep}]+)`)
        );
        ...
        if (profilesMatch) {
            ...
            const profileId = profilesMatch[1];
            if (!profileId) {
                throw new Error('Profile ID not found in match');
            }
    ...
    private detectActiveProfile(userDir: string): { id: string; name: string } | null {
        try {
            ...
            if (fs.existsSync(storageJsonPath)) {
                        ...
                        if (Array.isArray(submenuItems)) {
                            for (const item of submenuItems) {
                                if (
                                    ...
                                    const candidateId = item.command.replace(
                                        'workbench.profiles.actions.profileEntry.',
                                        ''
                                    );
                ...
                if (profileId) {
                    this.logger.debug(
                        `[CopilotSync] Profile detected from storage.json: ${profileId}`
                    );
            ...
            for (const candidateId of profiles) {
                ...
                if (fs.existsSync(profileGlobalStorage)) {
                    ...
                    if (ageMinutes < 5) {
                        this.logger.debug(
                            `[CopilotSync] Profile detected from filesystem heuristic: ${candidateId}`
                        );
     ...
     */
    private async syncSkillFromBundle(
        bundleId: string,
        bundlePath: string,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- TODO: Add proper types (Req 7)
        promptDef: any
    ...
    private async createCopilotFile(file: CopilotFile): Promise<void> {
        try {
            ...
            this.logger.info(
                `✅ Synced ${file.type}: ${file.name} → ${path.basename(file.targetPath)}`
            );
        } catch (error) {
    ...
    async unsyncBundle(bundleId: string): Promise<void> {
        try {
            ...
            if (!fs.existsSync(manifestPath)) {
                this.logger.warn(
                    `No manifest found for bundle: ${bundleId}, cannot determine files to remove`
                );
                return;
            }
                        ...
                        try {
                            if (fs.existsSync(copilotFile.sourcePath)) {
                                const targetContent = await readFile(
                                    copilotFile.targetPath,
                                    'utf-8'
                                );
                                const sourceContent = await readFile(
                                    copilotFile.sourcePath,
                                    'utf-8'
                                );
                                ...
                                if (normalizedTarget === normalizedSource) {
                                    await unlink(copilotFile.targetPath);
                                    this.logger.debug(
                                        `Removed copied file: ${path.basename(copilotFile.targetPath)}`
                                    );
                                    removedCount++;
                                } else {
                                    this.logger.warn(
                                        `Skipping modified file: ${path.basename(copilotFile.targetPath)}`
                                    );
                                }
                            } else {
                                this.logger.warn(
                                    `Skipping non-symlink file (source not found): ${path.basename(copilotFile.targetPath)}`
                                );
                            }
                        } catch (error) {
                            this.logger.warn(
                                `Failed to compare/remove file ${path.basename(copilotFile.targetPath)}: ${error}`
                            );
                        }
                    }
                }
    ...
    getCopilotSkillsDirectory(scope: 'user' | 'workspace' = 'user'): string {
        if (scope === 'workspace') {
            ...
            if (!workspaceFolders || workspaceFolders.length === 0) {
                throw new Error(
                    'No workspace folder open. Skills require an open workspace for workspace scope.'
                );
            }
            const firstFolder = workspaceFolders[0];
            if (!firstFolder) {
                throw new Error('Workspace folder is invalid');
            }
            return path.join(firstFolder.uri.fsPath, '.copilot', 'skills');
        }
    ...
    getClaudeSkillsDirectory(scope: 'user' | 'workspace' = 'user'): string {
        if (scope === 'workspace') {
            ...
            return path.join(firstFolder.uri.fsPath, '.claude', 'skills');
        }
    ...
    async unsyncSkill(
        skillName: string,
        scope: 'user' | 'workspace' = 'user',
        removeFromClaude: boolean = false
    ): Promise<void> {
        try {
