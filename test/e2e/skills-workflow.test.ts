...
`;
    ...
    const createCollectionWithSkill = (
        collectionId: string,
        skillPath: string
    ) => `id: ${collectionId}
    ...
    suite('Scaffold with Skills', () => {
        test('Scaffold awesome-copilot project includes skills directory', async function () {
            ...
            assert.ok(
                ...
                'SKILL.md should have description field'
            );
        });
    ...
    suite('Collection with Skills - Install Workflow', () => {
        test('Install bundle containing skill copies skill to correct location', async function () {
            ...
            nock('https://api.github.com')
                ...
                .reply(200, [{ name: 'skill-collection.collection.yml', type: 'file' }]);

            nock('https://raw.githubusercontent.com')
                .persist()
                .get(
                    '/test-owner/awesome-copilot-skills/main/collections/skill-collection.collection.yml'
                )
    ...
    suite('CopilotSyncService with Skills', () => {
        test('Sync recognizes skill files by pattern', async function () {
            ...
            const manifest = {
                ...
                version: '1.0.0',
                files: [{ path: 'skills/test-skill/SKILL.md', type: 'skill' }],
            };
