...
suite('Hub Source Loading', () => {
    ...
    suite('Source Loading Basics', () => {
        test('should load enabled sources from hub into registry', async () => {
            ...
            assert.ok(
                sources.some((s) => s.id === 'hub-test-hub-source-1'),
                'Should have hub-test-hub-source-1'
            );
            assert.ok(
                sources.some((s) => s.id === 'hub-test-hub-source-2'),
                'Should have hub-test-hub-source-2'
            );
        ...
        test('should skip disabled sources', async () => {
            ...
            assert.ok(
                mockRegistry.hasSource('hub-test-hub-disabled-enabled-source'),
                'Should have enabled source'
            );
            assert.ok(
                !mockRegistry.hasSource('hub-test-hub-disabled-disabled-source'),
                'Should not have disabled source'
            );
        });
        ...
        test('should update existing hub sources on re-import', async () => {
            ...
            assert.strictEqual(
                sourcesAfterFirst.length,
                2,
                'Should have 2 sources after first import'
            );
            ...
            assert.strictEqual(
                sourcesAfterReload.length,
                2,
                'Should still have only 2 sources (no duplicates)'
            );
            assert.strictEqual(
                mockRegistry.updateSourceCalls.length,
                2,
                'Should have 2 update calls'
            );
            assert.strictEqual(
                mockRegistry.addSourceCalls.length,
                0,
                'Should have 0 add calls on reload'
            );
        });
    });
    ...
    suite('Duplicate Detection', () => {
        test('should skip duplicate when URL and type match exactly', async () => {
            ...
            assert.strictEqual(
                sources.length,
                2,
                'Should have 2 sources (1 existing + 1 new, 1 duplicate skipped)'
            );
        ...
        test('should allow same URL with different collectionsPath', async () => {
            ...
            assert.strictEqual(
                sources.length,
                2,
                'Should have 2 sources (different collectionsPath)'
            );
        });
        ...
        test('should skip duplicate across multiple hubs', async () => {
            ...
            assert.strictEqual(
                sourcesAfterFirstHub.length,
                2,
                'Should have 2 sources from first hub'
            );
            ...
            assert.strictEqual(
                sourcesAfterSecondHub.length,
                2,
                'Should have only 2 sources (hub-b duplicates were skipped)'
            );
            ...
            assert.ok(
                !mockRegistry.hasSource('hub-hub-b-source-1'),
                'Should not have hub-b source-1 (duplicate)'
            );
            assert.ok(
                !mockRegistry.hasSource('hub-hub-b-source-2'),
                'Should not have hub-b source-2 (duplicate)'
            );
        });
    });
    ...
    suite('Integration with Hub Operations', () => {
        ...
        test('should reload sources when syncing hub', async () => {
            ...
            assert.ok(
                mockRegistry.hasSource('hub-test-sync-source-3'),
                'Should have the newly added source-3'
            );
        });
    });
});
