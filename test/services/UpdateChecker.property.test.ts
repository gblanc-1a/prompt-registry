        ...
        await fc.assert(
                ...
                async (bundleConfigs) => {
                    ...
                    const expectedUpdates = bundles.filter(b => {
                        ...
                        if (lMajor > iMajor) {return true;}
                        if (lMajor < iMajor) {return false;}
                        if (lMinor > iMinor) {return true;}
                        if (lMinor < iMinor) {return false;}
                ...
                async ({ bundles, subsequentCalls }) => {
                    ...
                    try {
                        ...
                        for (let i = 0; i < subsequentCalls; i++) {
                            ...
                            for (let j = 0; j < cachedResults.length; j++) {
                                if (cachedResults[j].bundleId !== firstResults[j].bundleId) {
                                    return false;
                                }
                                if (cachedResults[j].currentVersion !== firstResults[j].currentVersion) {
                                    return false;
                                }
                                if (cachedResults[j].latestVersion !== firstResults[j].latestVersion) {
                                    return false;
                                }
                            }
                        }
                ...
                async ({ bundles, hasNewUpdates }) => {
                    ...
                    try {
                        ...
                        if (hasNewUpdates && bundles.length > 0) {
                            ...
                            testRegistryManager.getBundleDetails.withArgs(newUpdates[newUpdates.length - 1].bundleId).resolves({
                                id: newUpdates[newUpdates.length - 1].bundleId,
                                name: newUpdates[newUpdates.length - 1].bundleId,
                                version: '2.0.0',
