...
import { UpdateScheduler, UpdateCheckFrequency } from '../../src/services/UpdateScheduler';
            ...
            fc.asyncProperty(
                ...
                async ({ numUpdates, enabled, frequency }) => {
                    ...
                    try {
                        ...
                        try {
                            // Mock configuration
                            const mockConfig = testSandbox.stub(
                                vscode.workspace,
                                'getConfiguration'
                            );
                            ...
                            const mockUpdates = new Array(numUpdates).fill(null).map((_, i) => ({
                                bundleId: `bundle-${i}`,
                        ...
                        try {
                            ...
                            for (let i = 0; i < numChecks; i++) {
                                ...
                                if (afterCallCount !== beforeCallCount + 1) {
                                    // Log for debugging
                                    console.log(
                                        `Check ${i}: Expected ${beforeCallCount + 1} calls, got ${afterCallCount}`
                                    );
                                    return false;
                                }
                            }
                            ...
                            if (actualCalls !== expectedTotalCalls) {
                                console.log(
                                    `Total: Expected ${expectedTotalCalls} calls, got ${actualCalls}`
                                );
                                return false;
                            }
        ...
        await fc.assert(
            fc.asyncProperty(
                fc
                    ...
                    .filter(
                        ({ initialFrequency, newFrequency }) => initialFrequency !== newFrequency
                    ),
                async ({ initialFrequency, newFrequency }) => {
                    ...
                    try {
                        ...
                        try {
                            ...
                            // Skip startup check
                            await testClock.tickAsync(5000);
                            const afterStartupCallCount =
                                testUpdateChecker.checkForUpdates.callCount;
