...
import { createMockInstalledBundle, createMockUpdateCheckResult } from './bundleTestHelpers';
...
export const AutoUpdateTestHelpers = {
    ...
    setupStorageMock(
        ...
        if (setResolves) {
            mockStorage.setUpdatePreference.withArgs(bundleId, sinon.match.any).resolves();
        }
    },
    ...
    assertPreferenceStored(
        ...
        assert.strictEqual(
            mockStorage.setUpdatePreference.getCall(callIndex).args[0],
            bundleId,
            `Expected bundleId '${bundleId}' at call ${callIndex}`
        );
        assert.strictEqual(
            mockStorage.setUpdatePreference.getCall(callIndex).args[1],
            expectedValue,
    ...
    assertPreferenceRetrieved(
        ...
        const assert = require('node:assert');
        assert.ok(
            mockStorage.getUpdatePreference.callCount > callIndex,
            `Expected getUpdatePreference to be called at least ${callIndex + 1} times`
        );
        assert.strictEqual(
            mockStorage.getUpdatePreference.getCall(callIndex).args[0],
            bundleId,
            `Expected bundleId '${bundleId}' at call ${callIndex}`
        );
    },
     ...
     */
    createTestInstalledBundle(bundleId: string, version: string, overrides?: any) {
