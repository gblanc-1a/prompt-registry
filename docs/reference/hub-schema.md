...
Engagement features configuration for telemetry, ratings, and user feedback.
...
| `ratings` | object | Rating configuration |
| `feedback` | object | Feedback configuration |
...
| `category` | string | Optional | Discussion category name for `github-discussions` backend (default: `Bundle Ratings`) |
...
| `anonymize` | boolean | Anonymize telemetry data |

### Ratings Object

| Field | Type | Description |
|-------|------|-------------|
| `enabled` | boolean | Enable rating features |
| `ratingsUrl` | string | URL to pre-computed `ratings.json` file (static hosting) |

### Feedback Object

| Field | Type | Description |
|-------|------|-------------|
| `enabled` | boolean | Enable feedback features |
| `requireRating` | boolean | Require rating when submitting feedback |
| `maxLength` | number | Maximum feedback comment length (100-10000) |

Feedback comments are submitted as comments on the corresponding GitHub Discussion. They are not aggregated into a hub-hosted file; readers can browse them directly on GitHub Discussions.
...
engagement:
  ...
  ratings:
    enabled: true
    ratingsUrl: https://raw.githubusercontent.com/myorg/prompt-registry-hub/main/ratings.json
  feedback:
    enabled: true
    requireRating: false
    maxLength: 2000
...
The `ratingsUrl` field points to a static JSON file that contains pre-computed rating aggregates.
...
This file is typically generated by an automated workflow (GitHub Actions) that lists discussions in the configured rating category and aggregates their star comments. See the [Engagement Guide](../user-guide/engagement.md) for details on setting up automated harvesting.
