...
suite('VersionConsolidator Unit Tests', () => {
    ...
    suite('consolidateBundles', () => {
        test('should consolidate 3 versions (1.0.0, 2.0.0, 1.5.0) into single entry with latest (2.0.0)', () => {
            ...
            assert.strictEqual(
                consolidated[0].isConsolidated,
                true,
                'Should be marked as consolidated'
            );
            assert.strictEqual(
                consolidated[0].availableVersions.length,
                3,
        ...
        test('should not consolidate single-version bundles', () => {
            const bundles = [BundleBuilder.github('owner', 'repo').withVersion('1.0.0').build()];
            ...
            assert.strictEqual(
                consolidated[0].isConsolidated,
                false,
                'Should not be marked as consolidated'
            );
    ...
    suite('setSourceTypeResolver', () => {
        test('should use custom source type resolver when provided', () => {
            ...
            assert.strictEqual(
                consolidated.length,
                2,
                'Should not consolidate with local source type'
            );
        });
    ...
    suite('LRU Cache Mechanism', () => {
        ...
        test('should evict least recently used entry when cache is full', async () => {
            ...
            assert.strictEqual(
                smallConsolidator.getAllVersions('owner1-repo1').length,
                0,
            ...
            assert.strictEqual(
                smallConsolidator.getAllVersions('owner2-repo2').length,
                1,
                'Recently used entry should remain'
            );
            assert.strictEqual(
                smallConsolidator.getAllVersions('owner3-repo3').length,
                1,
                'Recently used entry should remain'
            );
            assert.strictEqual(
                smallConsolidator.getAllVersions('owner4-repo4').length,
                1,
        ...
        test('should not evict entry when updating existing cache entry', () => {
            ...
            assert.strictEqual(
                smallConsolidator.getAllVersions('owner1-repo1').length,
                2,
                'Updated entry should have 2 versions'
            );
            assert.strictEqual(
                smallConsolidator.getAllVersions('owner2-repo2').length,
                1,
        ...
        test('should reject invalid cache sizes', () => {
            assert.throws(
                () => new VersionConsolidator(0),
                /positive number/,
                'Should reject zero'
            );
            assert.throws(
                () => new VersionConsolidator(-1),
                /positive number/,
                'Should reject negative'
            );
            assert.throws(
                () => new VersionConsolidator(Number.NaN),
                /positive number/,
                'Should reject NaN'
            );
            assert.throws(
                () => new VersionConsolidator(Infinity),
                /positive number/,
