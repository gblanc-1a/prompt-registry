...
suite('Bundle State Management - Integration Tests', () => {
    ...
    suite('Workflow 1: Install GitHub bundle → Uninstall → Verify UI shows Install button', () => {
        test('should show Install button after uninstalling GitHub bundle', async () => {
            ...
            assert.ok(
                mockStorage.removeInstallation.calledOnce,
                'Installation record should be removed'
            );
            ...
            const buttonState = determineButtonState(matchingInstalled?.version, bundle.version);

            assert.strictEqual(
                buttonState,
                'install',
                'Button state should be "install" after uninstall'
            );
        });
    });

    suite(
        'Workflow 2: Install v1.0.0 → Sync (v1.1.0 available) → Verify Update button shown',
        () => {
            test('should show Update button when newer version available after sync', async () => {
                ...
                assert.strictEqual(
                    buttonState,
                    'update',
                    'Button state should be "update" when newer version available'
                );
            });
        }
    );
    ...
    suite(
        'Workflow 3: Install v1.0.0 → Select v1.0.1 from dropdown → Verify v1.0.1 installed',
        () => {
            test('should install specific version when selected from dropdown', async () => {
                ...
                // Source retrieved via getSources()
                mockStorage.getSources.resolves([source]);
                mockStorage.getCachedSourceBundles
                    .withArgs('github-source')
                    .resolves([bundleV1_0_1]);
                ...
                assert.strictEqual(
                    installCalls.length,
                    1,
                    'Should have called installFromBuffer once'
                );
            });
        }
    );
    ...
    suite('Workflow 4: Sync GitHub source → Verify no auto-installation', () => {
        test('should NOT auto-install bundles when syncing GitHub source', async () => {
            ...
            assert.ok(mockStorage.cacheSourceBundles.calledOnce, 'Cache should be updated');
            assert.ok(
                mockInstaller.installFromBuffer.notCalled,
                'Should NOT auto-install bundles from GitHub source'
            );
            assert.ok(
                mockStorage.recordInstallation.notCalled,
                'Should NOT record any installations'
            );
            ...
            assert.strictEqual(
                installed.length,
                0,
                'No bundles should be auto-installed from GitHub source'
            );
        });
        ...
        test('should NOT auto-update installed bundles when syncing GitHub source', async () => {
            ...
            assert.strictEqual(
                installCallCount,
                0,
                'Should NOT auto-update bundles from GitHub source'
            );
            ...
            assert.strictEqual(
                matchingInstalled?.version,
                '1.0.0',
                'Version should remain 1.0.0 (not auto-updated)'
            );
        });
    });

    suite(
        'Workflow 5: Sync Awesome Copilot source → Verify auto-update of installed bundles',
        () => {
            test('should auto-update installed bundles when syncing Awesome Copilot source', async () => {
                ...
                mockStorage.getSources.resolves([source]);
                mockStorage.getCachedSourceBundles
                    .withArgs('awesome-copilot-source')
                    .resolves([bundleV1, bundleV1_1]);
                mockStorage.getInstalledBundles.resolves([installedBundle]);
            ...
            test('should NOT auto-update bundles from other sources when syncing Awesome Copilot source', async () => {
                ...
                mockStorage.getCachedSourceBundles
                    .withArgs('awesome-copilot-source')
                    .resolves([awesomeBundle]);
                mockStorage.getCachedSourceBundles
                    .withArgs('github-source')
                    .resolves([githubBundle]);
                mockStorage.getInstalledBundles.resolves([
                    installedAwesomeBundle,
                    installedGitHubBundle,
                ]);

                // Mock adapter returns updated awesome bundle
                const awesomeBundleV1_1 = BundleBuilder.fromSource(
                    'awesome-bundle',
                    'AWESOME_COPILOT'
                )
                    .withVersion('1.1.0')
                ...
                assert.strictEqual(
                    githubInstalled?.version,
                    '1.0.0',
                    'GitHub bundle version should remain unchanged'
                );
            });
        }
    );
    ...
    suite('Bug Fix: Install specific older version', () => {
        test('should install older version v1.0.16 when v1.0.17 is latest', async () => {
            ...
            const bundleV1_0_16 = BundleBuilder.github(
                'amadeus',
                'airlines-solutions-workflow-instructions'
            )
                .withVersion('1.0.16')
                .build();

            const bundleV1_0_17 = BundleBuilder.github(
                'amadeus',
                'airlines-solutions-workflow-instructions'
            )
                .withVersion('1.0.17')
            ...
            mockStorage.getCachedSourceBundles
                .withArgs('github-source')
                .resolves([bundleV1_0_17, bundleV1_0_16]);
            ...
            assert.strictEqual(
                matchingInstalled?.version,
                '1.0.16',
                'Installed version should be 1.0.16, not 1.0.17'
            );
            assert.strictEqual(
                matchingInstalled?.bundleId,
                bundleV1_0_16.id,
                'Bundle ID should match the requested version'
            );
            ...
            assert.strictEqual(
                installedBundle.id,
                bundleV1_0_16.id,
                'Should install v1.0.16, not v1.0.17'
            );
            assert.strictEqual(
                installedBundle.version,
                '1.0.16',
                'Bundle version should be 1.0.16'
            );
        });
    });
    ...
    suite('Bug Fix: Downgrade removes old version from display', () => {
        test('should show bug: v1.0.18 and v1.0.17 both installed after downgrade', async () => {
            ...
            assert.strictEqual(
                installed.length,
                2,
                'BUG REPRODUCED: Both 1.0.18 and 1.0.17 are installed'
            );
        ...
        test('should cleanup old version when downgrading through RegistryManager', async () => {
            ...
            const toRemove = allInstalled.filter(
                (b) =>
            ...
            assert.strictEqual(
                installed.length,
                1,
                'After downgrade, only one version should be installed'
            );
            assert.strictEqual(
                installed[0].version,
                '1.0.17',
                'Installed version should be 1.0.17'
            );
            assert.strictEqual(
                installed[0].bundleId,
                `${bundleIdBase}-1.0.17`,
                'Bundle ID should match v1.0.17'
            );
        });
    });
    ...
    suite('Rollback & Failure Recovery - Integration Tests', () => {
        test('should FAIL if cleanup happens BEFORE recording (catches the bug)', async () => {
            ...
            (mockStorage.recordInstallation as sinon.SinonStub).callsFake(
                async (bundle: InstalledBundle) => {
                    recordInstallationCalled = true;
                    installedBundles.set(bundle.bundleId, bundle);
                }
            );

            (mockStorage.removeInstallation as sinon.SinonStub).callsFake(
            ...
            (mockStorage.getInstalledBundles as sinon.SinonStub).callsFake(
                async (scope?: string) => {
            ...
            sandbox
                .stub(registryManager as any, 'resolveInstallationBundle')
                .resolves(bundleV1_0_18);
            sandbox
                .stub(registryManager as any, 'getSourceForBundle')
            ...
            await registryManager.installBundle(`${bundleIdBase}`, {
                scope: 'user',
                version: '1.0.18',
            });
            ...
            assert.ok(
                recordInstallationCalled,
                'recordInstallation must be called during installation'
            );
            ...
            assert.ok(
                removeInstallationCalled,
                'removeInstallation should be called during cleanup'
            );
            assert.ok(
                !installedBundles.has(`${bundleIdBase}-1.0.17`),
                'v1.0.17 should be cleaned up'
            );
        });
        ...
        test('should preserve old version when update fails during storage recording', async () => {
            ...
            try {
                ...
                assert.strictEqual(
                    bothInstalled.length,
                    2,
                    'After recording, both versions should exist'
                );
        ...
        test('should allow retry of failed update by checking if old version still exists', async () => {
            ...
            assert.strictEqual(
                afterCleanup.length,
                1,
                'After successful cleanup, only new version should remain'
            );
    ...
    suite('Bundle Details View: Opening details for installed versioned bundles', () => {
        test('should find bundle details when opening installed GitHub bundle with versioned ID (e.g., bundle-1.0.17)', async () => {
            ...
            const bundle = bundleBuilder.withVersion(bundleVersion).build();
            ...
            mockStorage.getCachedSourceBundles.withArgs('github-source').resolves([bundle]);
            mockStorage.getInstalledBundles.resolves([installedBundle]);
            mockStorage.getCachedBundleMetadata.resolves(); // No cache hit
