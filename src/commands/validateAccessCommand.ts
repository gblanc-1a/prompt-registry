...
export class ValidateAccessCommand {
        ...
        try {
            ...
            await vscode.window.withProgress(
                ...
                async (progress) => {
                    progress.report({
                        increment: 20,
                        message: 'Checking repository configuration...',
                    });
    ...
    private async showValidationResults(results: {
        ...
        statusItems.push(
            `**Connectivity**: ${connectivityIcon} ${results.connectivity ? 'Connected' : 'No connection'}`
        );

        // Access validation
        const accessIcon = results.accessValid ? '✅' : '❌';
        statusItems.push(
            `**Repository Access**: ${accessIcon} ${results.accessValid ? 'Valid' : 'Invalid'}`,
            `**Details**: ${results.accessMessage}`,
            ''
        );
        ...
        if (!results.connectivity) {
            statusItems.push(
                ...
                '- Verify GitHub API URL in settings'
            );
            ...
            if (results.usePrivateRepo && !results.hasToken) {
                statusItems.push(
                    ...
                    '- Configure token in VSCode settings: `promptregistry.githubToken`'
                );
            } else if (results.hasToken) {
                statusItems.push(
                    '- Verify token has correct permissions',
                    '- Check repository owner and name settings',
                    '- Ensure you have access to the repository'
                );
            } else {
                statusItems.push(
                    '- Verify repository owner and name are correct',
                    '- Check if repository is public or enable private mode'
                );
            }
        } else {
        ...
        if (results.accessValid && results.connectivity) {
            ...
            vscode.window
                .showWarningMessage(message, 'Open Settings', 'View Details')
                .then((selection) => {
                    if (selection === 'Open Settings') {
                        vscode.commands.executeCommand('workbench.action.openSettings', 'olaf');
                    }
