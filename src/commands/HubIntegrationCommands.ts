...
import { HubHistoryCommands } from './HubHistoryCommands';
import {
    activateHubProfile,
    deactivateHubProfile,
    showActiveProfiles,
} from './HubProfileActivationCommands';
import { HubSyncCommands } from './HubSyncCommands';

/**
 * Registers all hub integration commands
 */
export class HubIntegrationCommands {
    private syncCommands: HubSyncCommands;
    private historyCommands: HubHistoryCommands;
    ...
    private registerSyncCommands(): void {
        this.context.subscriptions.push(
            vscode.commands.registerCommand(
                'promptregistry.checkForUpdates',
                async (hubId?: string, profileId?: string) => {
                    try {
            ...
            vscode.commands.registerCommand(
                'promptregistry.viewProfileChanges',
                async (hubId?: string, profileId?: string) => {
                    try {
                        if (!hubId || !profileId) {
                            const profile = await this.selectActiveProfile();
            ...
            vscode.commands.registerCommand(
                'promptregistry.reviewAndSyncProfile',
                async (hubId?: string, profileId?: string) => {
                    try {
                        if (!hubId || !profileId) {
                            const profile = await this.selectActiveProfile();
