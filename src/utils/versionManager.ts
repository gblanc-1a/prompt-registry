...
import { SourceType } from '../types/registry';
...
export class VersionManager {
    ...
    static compareVersions(v1: string, v2: string): number {
        ...
        if (coerced1 && coerced2) {
            this.logger.debug(
                `Coerced versions for comparison: ${v1} -> ${coerced1.version}, ${v2} -> ${coerced2.version}`
            );
    ...
    static isUpdateAvailable(installedVersion: string, latestVersion: string): boolean {
        ...
        const cleanInstalled =
            semver.clean(installedVersion) || semver.coerce(installedVersion)?.version;
        ...
        this.logger.debug(
            `Using compareVersions fallback for update check: ${installedVersion} vs ${latestVersion}`
        );
