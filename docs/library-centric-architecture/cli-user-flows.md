...
**Goal**: Set up prompt-registry for a new project

```mermaid
flowchart LR
    A[Start] --> B[prompt-registry init]
    B --> C{Interactive?}
    C -->|Yes| D[Select IDE]
...
**Commands:**
- `prompt-registry init` (interactive or with flags)
...
**Commands:**
- `hub add --type github --url owner/repo` (or `--type local --url ./path`)
- `hub use <hub-id>`
- `hub sync <hub-id>`
...
- `discover --ai` (AI-powered discovery)
- `discover --interactive` (interactive selection)
...
| Command | Purpose | Key Options |
|---------|---------|------------|
| `bundle build` | Build bundle ZIP | `--repo-slug <owner/repo>`, `--out <dir>` |
| `bundle manifest` | Generate manifest | `--out <dir>` |

### Hub Commands
| Command | Purpose | Key Options |
|---------|---------|------------|
| `hub add <ref>` | Import hub | `--type <github\|local\|url>`, `--location <path>` |
...
| `discover` | Discover primitives | `--ai`, `--interactive`, `--kinds <kinds>`, `--limit <n>` |
