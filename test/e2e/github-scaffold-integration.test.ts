...
suite('E2E: GitHub Scaffold Integration Tests', () => {
    ...
    suite('Complete Scaffolding Flow', () => {
        ...
        test('E2E: Scaffold with custom project name substitutes variables correctly', async function () {
            ...
            assert.ok(readmeContent.includes(projectName), 'README should mention project name');
        });
...
suite('E2E: Script Execution Tests', () => {
    ...
    test('E2E: Validation script validates example collection successfully', async function () {
        ...
        assert.ok(fs.existsSync(validateScript), 'Validation script should exist');
    ...
    test('E2E: List collections script finds example collection', async function () {
        ...
        assert.ok(fs.existsSync(listScript), 'List collections script should exist');
    ...
    test('E2E: Build script creates bundle with correct structure', async function () {
        ...
        try {
            ...
            execSync(
                `node ${buildScript} --collection-file collections/example.collection.yml --version 1.0.0 --repo-slug test-repo --out-dir ${outputDir}`,
                {
                    cwd: testDir,
                    stdio: 'pipe',
                    timeout: 30_000,
            ...
            if (fs.existsSync(collectionOutDir)) {
                ...
                const hasManifest = outputFiles.some(
                    (f) => f.includes('manifest') || f.endsWith('.yml')
                );
                ...
                assert.ok(distFiles.length > 0, 'Build script should produce output files');
            }
        } catch (error: any) {
            const stderr = error.stderr?.toString() || '';
            const stdout = error.stdout?.toString() || '';
    ...
    test('E2E: Compute version script returns valid version', async function () {
        ...
        assert.ok(fs.existsSync(versionScript), 'Compute version script should exist');
    ...
    test('E2E: Pre-commit hook is properly configured', async function () {
        ...
        assert.ok(hookContent.startsWith('#!/'), 'Pre-commit hook should have shebang');
    ...
    test('E2E: Detect affected collections script is functional', async function () {
        ...
        assert.ok(fs.existsSync(detectScript), 'Detect affected collections script should exist');
    ...
    test('E2E: Generate manifest script is functional', async function () {
        ...
        assert.ok(fs.existsSync(manifestScript), 'Generate manifest script should exist');
    ...
    test('E2E: Validation library is properly configured', async function () {
        ...
        assert.ok(fs.existsSync(validateLib), 'Validation library should exist');
    ...
    test('E2E: Collections library is properly configured', async function () {
        ...
        assert.ok(fs.existsSync(collectionsLib), 'Collections library should exist');
