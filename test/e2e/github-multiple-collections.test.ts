...
`;
    ...
    suite('Bundle Detection', () => {
        test('should detect two separate bundles from two releases', async function () {
            ...
            assert.ok(
                bundles.length >= 2,
                `Should detect at least 2 bundles, got ${bundles.length}`
            );
    ...
    suite('Simultaneous Installation', () => {
        ...
        test('should maintain separate file structures', async function () {
            ...
            const wrongPromptA = path.join(
                recordA!.installPath,
                'prompts',
                'collection-b-prompt-1.md'
            );
            const wrongPromptB = path.join(
                recordB!.installPath,
                'prompts',
                'collection-a-prompt-1.md'
            );
            assert.ok(
                !fs.existsSync(wrongPromptA),
                'Collection-a should not have collection-b files'
            );
            assert.ok(
                !fs.existsSync(wrongPromptB),
    ...
    suite('Independent Uninstallation', () => {
        test('should uninstall collection-a without affecting collection-b', async function () {
            ...
            assert.ok(
                installed[0].bundleId.includes('collection-b'),
