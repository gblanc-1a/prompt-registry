...
const { normalizeRepoRelativePath } = require('./lib/validate');
...
function parseArgs(argv) {
  ...
  argv.forEach((arg, idx) => {
    if (arg === '--bundle-id' && argv[idx + 1]) {
      out.bundleId = argv[idx + 1];
    } else if (arg === '--collection-file' && argv[idx + 1]) {
...
try {
  ...
  const prompts = items.map(item => {
    ...
    if (!fs.existsSync(itemAbs)) {
      const normalizedNote =
        item.path && item.path !== itemPath ? ` (normalized: ${itemPath})` : '';
    ...
    const descMatch = itemContent.match(/^##?\s*Description[:\s]+(.+)$/im) || 
                     itemContent.match(/^>\s*(.+)$/m);
    ...
    return {
      id: path.basename(itemPath, extension),
      name: nameMatch ? nameMatch[1] : path.basename(itemPath, extension),
  ...
  let manifestId = args.bundleId;
  if (!manifestId) {
    // fallback to collection.id or package name
    manifestId = collection.id || packageJson.name.replace('@amadeus-airlines-solutions/', '');
  }
  ...
  const manifest = {
    ...
    author: collection.author || packageJson.author || 'Amadeus Airlines Solutions',
