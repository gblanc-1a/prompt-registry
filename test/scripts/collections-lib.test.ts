...
`
        ...
        test('handles optional fields', function () {
            ...
            writeFile(
                root,
                'collections/minimal.collection.yml',
                `
id: minimal
name: Minimal
items: []
`
            ...
            assert.strictEqual(
                collection.version,
                undefined,
                'version should be undefined when not specified'
            );
        });
        ...
        test('throws for invalid YAML', function () {
            ...
            writeFile(
                root,
                'collections/invalid.collection.yml',
                `
id: test
name: Test
    ...
    suite('resolveCollectionItemPaths()', () => {
        ...
        test('normalizes Windows-style paths', function () {
            ...
            const collection = {
                id: 'test',
                name: 'Test',
                items: [{ path: String.raw`prompts\windows\style.md`, kind: 'prompt' }],
            };

            const paths = collectionsLib.resolveCollectionItemPaths(root, collection);

            assert.strictEqual(paths.length, 1);
            assert.ok(!paths[0].includes('\\'), 'Paths should use forward slashes');
        });
        ...
        test('includes all files in skill directory when kind is skill', function () {
            ...
            assert.ok(
                paths.length >= 4,
                `Should include all skill files, got ${paths.length}: ${JSON.stringify(paths)}`
            );
            assert.ok(paths.includes('skills/my-skill/SKILL.md'), 'Should include SKILL.md');
            assert.ok(
                paths.includes('skills/my-skill/assets/diagram.png'),
                'Should include assets'
            );
            assert.ok(
                paths.includes('skills/my-skill/references/doc.md'),
                'Should include references'
            );
            assert.ok(
                paths.includes('skills/my-skill/scripts/helper.js'),
        ...
        test('includes skill directory files alongside regular prompts', function () {
            ...
            assert.ok(paths.includes('skills/my-skill/SKILL.md'), 'Should include SKILL.md');
            assert.ok(
                paths.includes('skills/my-skill/assets/image.png'),
                'Should include skill assets'
            );
        ...
        test('enables detection of changes to skill subdirectory files', function () {
            ...
            writeFile(
                root,
                'skills/github-issues-triage/scripts/triage_issues.py',
                'print("triage")'
            );
            ...
            const collection = {
                id: 'amadeus-ospo',
                name: 'Amadeus OSPO',
                items: [{ path: 'skills/github-issues-triage/SKILL.md', kind: 'skill' }],
            };
            ...
            assert.ok(
                itemPathsSet.has('skills/github-issues-triage/assets/report.template.md'),
                'Should include assets files'
            );
            assert.ok(
                itemPathsSet.has('skills/github-issues-triage/references/EXAMPLES.md'),
