...
suite('LocalOlafAdapter', () => {
    ...
    suite('competency index path consistency', () => {
        ...
        test('should use consistent paths for installation and uninstallation', async () => {
            ...
            adapter['ensureRuntimeInstalled'] = async () => {
                /* mock - do nothing */
            };
            adapter['createWorkspaceLinks'] = async () => {
                /* mock - do nothing */
            };
            adapter['createSkillSymbolicLinks'] = async () => {
                /* mock - do nothing */
            };
            adapter['removeSkillSymbolicLinks'] = async () => {
                /* mock - do nothing */
            };
            ...
            try {
                ...
                assert.strictEqual(
                    entry.file,
                    'external-skills/Test Local OLAF/test-skill/prompts/test.md',
                ...
                const updatedIndexContent = JSON.parse(
                    fs.readFileSync(competencyIndexPath, 'utf-8')
                );
                assert.ok(
                    Array.isArray(updatedIndexContent),
                    'Competency index should still be an array'
                );
                assert.strictEqual(
                    updatedIndexContent.length,
                    0,
                    'Entry should be removed during uninstallation'
                );
            } finally {
