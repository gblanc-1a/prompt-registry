...
suite('RepositoryScopeService Property Tests', () => {
    ...
    suite('Property: Skill Directory Preservation', function() {
         ...
         */
        const skillStructureGen = (): fc.Arbitrary<{ skillName: string; files: Array<{ relativePath: string; content: string }> }> => {
            return fc.tuple(
                skillNameGen(),
                fc.array(
                    fc.tuple(
...
prompts:
  - id: ${skillName}
    ...
    type: skill`;
         ...
         */
        const readDirectoryRecursive = (dir: string, basePath: string = ''): Array<{ relativePath: string; content: string }> => {
