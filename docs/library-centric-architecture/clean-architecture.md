...
This document explains how the prompt-registry packages follow Clean Architecture principles.
...
The prompt-registry packages follow Clean Architecture principles:

### Domain Layer (`@prompt-registry/core`)

- Contains pure domain types: `Bundle`, `Collection`, `Primitive`, `Target`, `Source`
- Defines port interfaces in `ports/`: `IBundleDownloader`, `IFilesystem`, `IHttpClient`, `IGitHubApi`, `ITargetWriter`
- No dependencies on other packages or external infrastructure
- Business rules live here (validation, parsing, domain logic)

### Infrastructure Layer (`@prompt-registry/infra`)

- Implements ports as adapters: `HttpsBundleDownloader`, `NodeHttpClient`, `GitHubClient`, `FileTreeTargetWriter`
- Provides concrete implementations for external systems: GitHub API, file system, HTTP, ZIP extraction
- Depends only on `@prompt-registry/core` (the port interfaces)
- Can be swapped without affecting domain logic

### Application Layer (`@prompt-registry/app`)

- Orchestrates use cases: profile activation, bundle installation, registry management
...
### CLI Layer (`@prompt-registry/cli`)

- Thin adapter that translates CLI commands into application use cases
- Handles I/O, user interaction, and presentation
- Depends on app, core, and infra but domain remains isolated
- Can be replaced with a web UI or SDK without changing core
...
- Add new delivery mechanisms (CLI, web UI, SDK) without changing domain
