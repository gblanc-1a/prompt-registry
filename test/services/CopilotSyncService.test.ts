...
suite('CopilotSyncService', () => {
    ...
    setup(() => {
        ...
        mockContext = {
            globalStorageUri: {
                fsPath: path.join(mockUserDir, 'globalStorage', 'publisher.extension'),
            },
    ...
    suite('Service Initialization', () => {
        ...
        test('should have sync methods', () => {
            ...
            assert.ok(
                typeof service.unsyncBundle === 'function',
                'Should have unsyncBundle method'
            );
    ...
    suite('Path Resolution', () => {
        ...
        const pathTestCases = [
            {
                name: 'macOS - Kiro',
                globalStoragePath:
                    '/Users/testuser/Library/Application Support/Kiro/User/globalStorage/publisher.extension',
                expectedPath: '/Users/testuser/Library/Application Support/Kiro/User/prompts',
            },
            {
                name: 'macOS - VS Code',
                globalStoragePath:
                    '/Users/testuser/Library/Application Support/Code/User/globalStorage/publisher.extension',
                expectedPath: '/Users/testuser/Library/Application Support/Code/User/prompts',
            },
            {
                name: 'macOS - VS Code Insiders',
                globalStoragePath:
                    '/Users/testuser/Library/Application Support/Code - Insiders/User/globalStorage/publisher.extension',
                expectedPath:
                    '/Users/testuser/Library/Application Support/Code - Insiders/User/prompts',
            },
            {
                name: 'macOS - Windsurf',
                globalStoragePath:
                    '/Users/testuser/Library/Application Support/Windsurf/User/globalStorage/publisher.extension',
                expectedPath: '/Users/testuser/Library/Application Support/Windsurf/User/prompts',
            },
            {
                name: 'macOS - Cursor',
                globalStoragePath:
                    '/Users/testuser/Library/Application Support/Cursor/User/globalStorage/publisher.extension',
                expectedPath: '/Users/testuser/Library/Application Support/Cursor/User/prompts',
            },
            {
                name: 'Linux - VS Code',
                globalStoragePath:
                    '/home/testuser/.config/Code/User/globalStorage/publisher.extension',
                expectedPath: '/home/testuser/.config/Code/User/prompts',
            },
            {
                name: 'Linux - Custom IDE',
                globalStoragePath:
                    '/home/user/.config/CustomIDE/User/globalStorage/com.company.extension',
                expectedPath: '/home/user/.config/CustomIDE/User/prompts',
            },
        ];

        for (const { name, globalStoragePath, expectedPath } of pathTestCases) {
            test(`should resolve prompts directory - ${name}`, async () => {
                ...
                assert.strictEqual(
                    status.copilotDir,
                    expectedPath,
                    `Should resolve correct path for ${name}`
                );
            });
        }
        ...
        const profileTestCases = [
            {
                ...
                globalStoragePath:
                    '/Users/testuser/Library/Application Support/Code/User/profiles/3a5f32f8/globalStorage/publisher.extension',
                expectedPath:
                    '/Users/testuser/Library/Application Support/Code/User/profiles/3a5f32f8/prompts',
            },
            {
                name: 'Linux with profile',
                globalStoragePath:
                    '/home/testuser/.config/Code/User/profiles/xyz789/globalStorage/publisher.extension',
                expectedPath: '/home/testuser/.config/Code/User/profiles/xyz789/prompts',
            },
        ];

        for (const { name, globalStoragePath, expectedPath } of profileTestCases) {
            test(`should resolve prompts directory - ${name}`, async () => {
                const testContext = {
                    globalStorageUri: { fsPath: globalStoragePath },
                    storageUri: { fsPath: tempDir },
        ...
        test('should resolve prompts directory - Windows with profile (cross-platform test)', async () => {
            const winProfileBase = path.join(
                tempDir,
                'WindowsProfile',
                'Code',
                'User',
    ...
    suite('Windows Path Regex Handling (Backslash Escaping)', () => {
        ...
        test('should handle Windows-style path with backslashes - profile-based', async () => {
            ...
            assert.strictEqual(
                status.copilotDir,
                expectedPath,
                'Should resolve to profile prompts directory'
            );
        });
        ...
        test('should handle Windows-style path - custom data directory with profile', async () => {
            ...
            assert.strictEqual(
                status.copilotDir,
                expectedPath,
                'Should resolve custom data dir with profile'
            );
        });
    ...
    suite('getStatus', () => {
        test('should return status information', async () => {
            ...
            assert.ok(
                typeof status.dirExists === 'boolean',
                'Should include directory existence flag'
            );
        ...
        test('should report copilot directory path', async () => {
            ...
            assert.ok(
                status.copilotDir.includes('User') || status.copilotDir.includes('prompts'),
    ...
    suite('Behavior Validation', () => {
        test('should return valid absolute path', async () => {
            ...
            assert.ok(
                ...
                'Path should end with prompts directory'
            );
        });
        ...
        test('should handle path with User directory', async () => {
            ...
            assert.ok(
                ...
                'Should include User directory in test setup'
            );
        });
    ...
    suite('Error Handling', () => {
        test('should handle missing deployment manifest', async () => {
            ...
            try {
                ...
                assert.ok(
                    error.message.includes('manifest') || error.message.includes('ENOENT'),
        ...
        suite('Profile Detection Workarounds', () => {
            ...
            test('should prefer storage.json over filesystem heuristic when both available', async () => {
                ...
                const oldProfileGlobalStorage = path.join(
                    profilesDir,
                    oldProfileId,
                    'globalStorage'
                );
            ...
            suite('WSL Support', () => {
                test('should detect WSL remote and return Windows mount path when globalStorage is on /mnt/c', async () => {
                    ...
                    const originalEnv = (globalThis as any).vscode?.env;
                    if (!(globalThis as any).vscode) {
                        (globalThis as any).vscode = {};
                    }
                    (globalThis as any).vscode.env = {
                        remoteName: 'wsl',
                    ...
                    try {
                        ...
                        assert.ok(
                            status.copilotDir.startsWith(
                                '/mnt/c/Users/testuser/AppData/Roaming/Code/User'
                            ),
                        ...
                        if (originalEnv) {
                            (globalThis as any).vscode.env = originalEnv;
                        } else {
                ...
                test('should handle WSL with D: drive mount', async () => {
                    ...
                    try {
                        ...
                        if (originalEnv) {
                            ...
                            delete (globalThis as any).vscode?.env;
                        }
                    }
                });
